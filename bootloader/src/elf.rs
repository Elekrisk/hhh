use core::{mem::MaybeUninit::{self}, slice};
...
impl<'a> Elf<'a> {
    pub fn parse(data: &'a [u8]) -> Result<Elf<'a>, ()> {
        // Magic
        if data[0..4] != [0x7F, 0x45, 0x4C, 0x46] { return Err(()) }

        // 32- or 64-bit
        if data[4] != 2 { return Err(()) }
        // Endianness
        if data[5] != 1 { return Err(()) }
        // Version
        if data[6] != 1 { return Err(()) }
        
        let abi = if data[7] == 0 { Abi::SystemV } else { Abi::Other };
        ...
        // Machine
        if &data[0x12..0x14] != &0x3Eu16.to_le_bytes() { return Err(()) }
        // Version, again
        if &data[0x14..0x18] != &1u32.to_le_bytes() { return Err(()) }
        ...
        let section_name_section_index = u16::from_le_bytes(slice_to_array(&data[0x3E..0x40])) as usize;
        ...
        for i in 0..program_header_count {
            ...
            let data_offset = u64::from_le_bytes(data[offset+0x8..offset+0x10].try_into().unwrap());
            ...
            let virtual_addr  = u64::from_le_bytes(data[offset+0x10..offset+0x18].try_into().unwrap());
            let physical_addr = u64::from_le_bytes(data[offset+0x18..offset+0x20].try_into().unwrap());
            let file_size = u64::from_le_bytes(data[offset+0x20..offset+0x28].try_into().unwrap());
            let mem_size = u64::from_le_bytes(data[offset+0x28..offset+0x30].try_into().unwrap());
        ...
        for i in 0..section_header_count {
            ...
            let offset = offset as usize;
            let section_type = match u32::from_le_bytes(data[offset+4..offset+8].try_into().unwrap()) {
                0 => SectionType::None,
            ...
            let virtual_addr  = u64::from_le_bytes(data[offset+0x10..offset+0x18].try_into().unwrap());
            let data_offset = u64::from_le_bytes(data[offset+0x18..offset+0x20].try_into().unwrap());
            let file_size = u64::from_le_bytes(data[offset+0x20..offset+0x28].try_into().unwrap());
            ...
            let align = u64::from_le_bytes(data[offset+0x30..offset+0x38].try_into().unwrap());
            let entry_size = u64::from_le_bytes(data[offset+0x38..offset+0x40].try_into().unwrap());
