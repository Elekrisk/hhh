...
static FONT: &[[[u8; 8]; 16]; 128] =
    unsafe { core::mem::transmute(include_bytes!("../vgafont.bin")) };
        ...
        for y in 0..self.framebuffer.resolution_y {
            unsafe {
                core::ptr::write_bytes(
                    self.framebuffer.ptr.add(y * self.framebuffer.stride),
                    0,
                    self.framebuffer.resolution_x,
                );
            }
        }
    }
            ...
            _ => {
                for y in 0..16 {
                    for x in 0..8 {
                        ...
                        let color = ((grayscale as u32) << 16)
                            | ((grayscale as u32) << 8)
                            | grayscale as u32;
                        unsafe {
                            base.add((cy * 16 + y) * self.framebuffer.stride + cx * 8 + x)
                                .write(color);
                        }
                    }
                }
    ...
    fn scroll(&mut self) {
        for y in 0..self.framebuffer.resolution_y - 16 {
            ...
            for x in 0..res_x {
                unsafe {
                    base.add(y * stride + x)
                        .write_volatile(base.add((y + 16) * stride + x).read_volatile());
                }
            }
            // unsafe {
